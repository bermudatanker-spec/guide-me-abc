...
type ListingRow = {
  id: string;
  owner_id: string; // BELANGRIJK: kolom heet owner_id in je tabel
...
export default function EditBusinessPage() {
  ...
  const [form, setForm] = useState<{
    business_name: string;
    island: "aruba" | "bonaire" | "curacao" | "";
    category_id: string | ""; // leeg string = geen categorie
    description: string;
    address: string;
    phone: string;
    email: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // 1) Auth check (voor de zekerheid, RLS beschermt ook)
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          island: data.island,
          category_id: data.category_id, // null of uuid
      ...
      toast({
        title: "Fout",
        description:
          err?.message ?? "Er ging iets mis bij het opslaan van je wijzigingen.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
